...
Single training driver for BOTH modes:
  * single-task (default) — the Mode A loop below; what the Studio bridge spawns.
  * multi-task zero-shot generalization — task-conditioned world model + policy,
    reached with `multitask=true` (see `run_multitask`). `scripts/train_multitask.py`
    is a thin shim that flips that flag and calls `run_training` here.
...
Usage:
  ...
  python scripts/train.py multitask=true env=halfcheetah_vel   # multi-task run
"""
...
def eval_zero_shot(trainer, family: str, taus: list[float], device, seed: int,
    ...
    """Multi-task zero-shot eval: roll the task-conditioned policy on each held-out
    tau and return {tau: mean episode return}. Used by `run_multitask`."""
...
def run_single_task(cfg: DictConfig, device: str):
    """The single-task Mode A loop. Byte-identical to the historical `main` body
    (device already resolved and `seed_everything` already called by the caller).
    This is the path the Studio bridge spawns via `scripts/train.py <overrides>`."""
...
def run_multitask(cfg: DictConfig, device: str):
    """Multi-task zero-shot generalization (validation item 9).
    ...
    ablation is the same command with penalty.schedule.lam0=0. Reached via
    `multitask=true` (or the `scripts/train_multitask.py` shim). Device already
    resolved and `seed_everything` already called by the caller."""
...
def run_training(cfg: DictConfig):
    """Shared driver for both entry points. Resolves the device and seeds EXACTLY
    as the historical single-task `main` did, then dispatches: multi-task when
    `cfg.multitask` is truthy (default false ⇒ unchanged single-task path)."""
    device = _resolve_device(cfg)
    seed_everything(cfg.seed)
    if cfg.get("multitask", False):
        run_multitask(cfg, device)
    else:
        run_single_task(cfg, device)
