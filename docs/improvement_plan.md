...
2. **[ready] Recalibration-on-drift for sigma_w=auto.** sigma* is measured on
...
3. **[ready] Weight decay on learned log_s.** The learned-sigma gradient has an
...
4. **[ready] Per-band SNR as a logged diagnostic everywhere.** weights_mode=snr
...
5. **[ready] Deconfound angle-vs-bandwidth (run 2 caveat).** Anisotropic W
...
8. **[ready] Quarantine integration tests.** The suite is creeping past 30s
...
9. **[ready] Preset registry dedup.** parallel_runs.py PRESETS has grown
...
10. **[ready] One-command experiment status.** `scripts/status.py`: read
...
11. **Single source of truth for the recipe rules.** The spectral rules
...
12. **Colab cell-2 install check.** The MuJoCo failure mode (pip line silently
    truncated by `| tail -1`) cost a 9-run batch. Replace with an assert-import
    cell that fails fast and says what to do.

## C. Writeup readiness

13. **Results provenance.** bridge_experiment/recipe results overwrite in
...
14. **Auto-generated ledger table.** The per-run numbers in claims_ledger.md
